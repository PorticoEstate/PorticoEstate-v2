...
<xsl:template match="data">
	<xsl:choose>
		...
		<xsl:when test="convert_to_eav">
			<xsl:apply-templates select="convert_to_eav" />
		</xsl:when>
...
<!-- New template-->
<xsl:template match="convert_to_eav" xmlns:php="http://php.net/xsl">
	<xsl:variable name="form_action">
		<xsl:value-of select="form_action" />
	</xsl:variable>
	<form method="post" class="pure-form pure-form-aligned" action="{$form_action}">
			...
			<tbody>
				<xsl:for-each select="non_eav">
					<tr>
						...
						<td align="center">
							<input type="checkbox" name="values[locations][]" value="{location}">
								<xsl:attribute name="title">
									<xsl:value-of select="location" />
								</xsl:attribute>
							</input>
		...
		<div class="pure-control-group">
			<xsl:variable name="lang_convert">
				<xsl:value-of select="php:function('lang', 'convert')" />
			</xsl:variable>
			<input type="submit" class="pure-button pure-button-primary" name="values[confirm_convert_to_eav]" value="{$lang_convert}">
				<xsl:attribute name="title">
					<xsl:value-of select="php:function('lang', 'convert_statustext')" />
				</xsl:attribute>
			</input>
		</div>
	</form>
	<div class="pure-control-group">
		...
		<form method="post" action="{$done_action}">
			<input type="submit" class="pure-button pure-button-primary" name="done" value="{$lang_done}">
				<xsl:attribute name="title">
					<xsl:value-of select="php:function('lang', 'done_statustext')" />
				</xsl:attribute>
			</input>
		</form>
	</div>
	<script>
		<![CDATA[
			document.getElementById('select_all').addEventListener('change', function() {
				var checkboxes = document.querySelectorAll('input[name="values[locations][]"]');
