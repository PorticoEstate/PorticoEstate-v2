...
class calendar_socalendar
{
	...
	function __construct($param)
	{
		...
		$user_account_id = is_array($user_settings) ? ($user_settings['account_id'] ?? 0) : 0;
		$this->owner = (!isset($param['owner']) || $param['owner'] == 0 ? $this->accounts_obj->search_person($user_account_id) : $param['owner']);
	...
	function list_repeated_events($syear, $smonth, $sday, $eyear, $emonth, $eday, $owner_id = 0)
	{
		...
		if (!is_array($server_settings) || ($server_settings['calendar_type'] ?? null) != 'sql')
		{
			return array();
		}
