...
class booking_socompleted_reservation_export extends booking_socommon
{
	...
	function add($entry)
	{
		...
		$entry['total_items'] = count(array_filter($export_reservations, array(
			$this,
			'not_free'
		)));
	...
	function archive($entry)
	{
		...
		$receipt = parent::add($entry);
		$entry['id'] = $receipt['id'];
		$this->update_completed_reservations_archived_state($entry, $export_reservations);
	...
	public function &get_completed_reservations_for($entity)
	{
		...
		$reservations = $this->completed_reservation_so->read(array(
			'filters' => $filters,
			'results' => 'all',
			'sort' => 'customer_type,customer_identifier_type,customer_organization_number,customer_number,customer_ssn,to_',
			'dir' => 'asc'
		));
	...
	protected function update_completed_reservations_archived_state($entity, &$reservations)
	{
		return $this->completed_reservation_so->update_archived_state_of($reservations, $entity['id']);
	}
			...
			if (count($external_reservations = array_filter($reservations, array($this, 'select_external'))) > 0)
			{
				...
				if ($this->config_data['external_format'] == 'CSV')
				{
					return $this->build_export_result(
						...
						$this->format_csv($external_reservations, $account_codes, $number_generator)
					);
				}
				elseif ($this->config_data['external_format'] == 'AGRESSO')
				{
				...
				elseif ($this->config_data['external_format'] == 'FACTUM')
				{
					return $this->build_export_result(
						...
						count(array_filter($external_reservations, array($this, 'not_free'))),
						$this->calculate_total_cost($external_reservations),
						$this->format_factum($external_reservations, $account_codes, $number_generator)
					);
				}
				elseif ($this->config_data['external_format'] == 'KOMMFAKT')
				{
			...
			if (count($internal_reservations = array_filter($reservations, array($this, 'select_internal'))) > 0)
			{
				...
				elseif ($this->config_data['internal_format'] == 'FACTUM')
				{
					return $this->build_export_result(
						...
						count(array_filter($internal_reservations, array($this, 'not_free'))),
						$this->calculate_total_cost($internal_reservations),
						$this->format_factum($internal_reservations, $account_codes, $number_generator)
					);
				}
				elseif ($this->config_data['internal_format'] == 'KOMMFAKT')
				{
	...
	public function create_export_item_info(&$entity, $generated_order_id)
	{
		...
		return array(
			...
			'invoice_file_order_id' => $generated_order_id
		);
	}
...
public function format_agresso_55(array &$reservations, array $account_codes, $sequential_number_generator)
{
	...
	protected function get_agresso_row_template()
	{
		...
		$row_template = array(
			...
			'art_descr' => str_repeat(' ', 35),
			'article' => str_repeat(' ', 15),
			'att_1_id' => str_repeat(' ', 2),
			'att_2_id' => str_repeat(' ', 2),
			'att_3_id' => str_repeat(' ', 2),
			'att_4_id' => str_repeat(' ', 2),
			'att_5_id' => str_repeat(' ', 2),
			'att_6_id' => str_repeat(' ', 2),
...
//			'client_ref' => str_repeat(' ', 2), // slått sammen med client for å få 4 posisjoner i 'client'
			...
			'currency' => str_repeat(' ', 3),
			'del_met_descr' => str_repeat(' ', 60),
			'del_term_descr' => str_repeat(' ', 60),
			'deliv_addr' => str_repeat(' ', 255),
			'deliv_attention' => str_repeat(' ', 50),
			'deliv_countr' => str_repeat(' ', 3),
			'deliv_date' => str_repeat(' ', 17),
			'deliv_method' => str_repeat(' ', 8),
			'deliv_terms' => str_repeat(' ', 8),
			'dim_1' => str_repeat(' ', 8),
			'dim_2' => str_repeat(' ', 8),
			'dim_3' => str_repeat(' ', 8),
			'dim_4' => str_repeat(' ', 8),
			...
			'dim_6' => str_repeat(' ', 4),
			'dim_7' => str_repeat(' ', 4),
			'dim_value_1' => str_repeat(' ', 12),
			'dim_value_2' => str_repeat(' ', 12),
			'dim_value_3' => str_repeat(' ', 12),
			'dim_value_4' => str_repeat(' ', 12),
			'dim_value_5' => str_repeat(' ', 12),
			'dim_value_6' => str_repeat(' ', 12),
			'dim_value_7' => str_repeat(' ', 12),
			'disc_percent' => str_repeat(' ', 17),
			'exch_rate' => str_repeat(' ', 17),
			'ext_ord_ref' => str_repeat(' ', 15),
			'intrule_id' => str_repeat(' ', 6),
			'line_no' => str_repeat(' ', 4),
			'location' => str_repeat(' ', 4),
			'long_info1' => str_repeat(' ', 120),
			'long_info2' => str_repeat(' ', 120),
			'lot' => str_repeat(' ', 10),
			'main_apar_id' => str_repeat(' ', 8),
			'mark_attention' => str_repeat(' ', 50),
			'mark_ctry_cd' => str_repeat(' ', 3),
			'markings' => str_repeat(' ', 120),
			'obs_date' => str_repeat(' ', 17),
			'order_date' => str_repeat(' ', 17),
			'order_id' => str_repeat(' ', 9),
			'order_type' => str_repeat(' ', 2),
			'pay_method' => str_repeat(' ', 2),
			'period' => str_repeat(' ', 8),
			'place' => str_repeat(' ', 30),
			'province' => str_repeat(' ', 40),
			'rel_value' => str_repeat(' ', 12),
			'responsible' => str_repeat(' ', 8),
			'responsible2' => str_repeat(' ', 8),
			'sequence_no' => str_repeat(' ', 8),
			'sequence_ref' => str_repeat(' ', 8),
			'serial_no' => str_repeat(' ', 20),
			'short_info' => str_repeat(' ', 60),
			'status' => str_repeat(' ', 1),
			'tax_code' => str_repeat(' ', 2),
			'tax_system' => str_repeat(' ', 2),
			'template_id' => str_repeat(' ', 8),
			'terms_id' => str_repeat(' ', 2),
			'tekst1' => str_repeat(' ', 12),
			'tekst2' => str_repeat(' ', 12),
			'tekst3' => str_repeat(' ', 12),
			'tekst4' => str_repeat(' ', 12),
			'trans_type' => str_repeat(' ', 2),
			'unit_code' => str_repeat(' ', 3),
			'unit_descr' => str_repeat(' ', 50),
			'value_1' => str_repeat(' ', 17),
