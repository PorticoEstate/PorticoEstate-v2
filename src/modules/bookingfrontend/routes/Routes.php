...
$app->group('/bookingfrontend', function (RouteCollectorProxy $group)
{
	...
	$group->group('/organizations', function (RouteCollectorProxy $group)
	{
		...
		$group->group('/{id}', function (RouteCollectorProxy $group) {
			$group->get('/activities', OrganizationController::class . ':getSubActivityList');
			...
			$group->patch('', OrganizationController::class . ':update');
	...
	->add(new SessionsMiddleware($group->getContainer()))
