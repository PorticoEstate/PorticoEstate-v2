	...
	public function get_menu($type = '')
	{
			...
			$admin_children_tenant = array(
				...
				'tenant_attribs'	 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.tenant',
						'menu_selection' => 'admin::property::tenant::tenant_attribs'
					))
				),
				'claims_cats'		 => array(
			...
			$admin_children_vendor = array(
				...
				'vendor_attribs'	 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.vendor',
						'menu_selection' => 'admin::property::vendor::vendor_attribs'
					))
				)
			);
			...
			$admin_children_project = array(
				...
				'project_attribs'				 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.project',
						'menu_selection' => 'admin::property::project::project_attribs'
					))
				),
				...
				'project_functions'				 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.project',
						'menu_selection' => 'admin::property::project::project_functions'
					))
				),
			...
			$admin_children_ticket = array(
				...
				'ticket_attribs'	 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.ticket',
						'menu_selection' => 'admin::property::ticket::ticket_attribs'
					))
				),
				'ticket_functions'	 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.ticket',
						'menu_selection' => 'admin::property::ticket::ticket_functions'
					))
				),
			);
			...
			$admin_children_agreement = array(
				...
				'agreement_attribs'			 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.agreement',
						'menu_selection' => 'admin::property::agreement::agreement_attribs'
					))
				),
				...
				'service_agree_attribs'		 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'menu_selection' => 'admin::property::agreement::service_agree_attribs'
					))
				),
				'service_agree_item_attribs' => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'menu_selection' => 'admin::property::agreement::service_agree_item_attribs'
					))
				)
			);
			...
			$admin_children_owner = array(
				...
				'owner_attribs'	 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.owner',
						'menu_selection' => 'admin::property::owner::owner_attribs'
					))
				)
			);
		}
		...
		if ($sysadmin || $local_admin)
		{
			...
			$admin_children_location['cadastre']			 = array(
				...
				'url'	 => phpgw::link('/index.php', array(
					'menuaction'	 => 'admin.ui_custom.list_attribute',
					'appname'		 => 'property',
					'location'		 => '.location.gab',
					'menu_selection' => 'admin::property::location::cadastre'
				))
			);
			...
			$menus['admin'] = array(
				...
				'condition_survey_cats'		 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						'menuaction'	 => 'admin.uicategories.index',
						'appname'		 => 'property',
						'location'		 => '.project.condition_survey',
						'global_cats'	 => 'true',
				...
				'request_attribs'			 => array(
					...
					'url'	 => phpgw::link('/index.php', array(
						...
						'appname'		 => 'property',
						'location'		 => '.project.request',
						'menu_selection' => 'admin::property::request_attribs'
					))
				),
				'order_dim1'				 => array(
		...
		if (isset($userSettings['apps']['preferences']))
		{
			$menus['preferences'] = array(
				...
				array(
					'text'	 => lang('substitute'),
					'url'	 => phpgw::link('/index.php', array(
						'menuaction' => 'property.uisubstitute.edit',
						'acl_app'	 => 'property'
					))
				),
			...
			if (!isset($config['suppress_tenant']) || !$config['suppress_tenant'])
			{
				$children['tenant'] = array(
					'url'	 => phpgw::link('/index.php', array(
						'menuaction'	 => 'property.uilocation.index',
						'lookup_tenant'	 => 1,
						'type_id'		 => $soadmin_location->read_config_single('tenant_id')
					)),
					'text'	 => lang('Tenant'),
					'image'	 => array('property', 'location_tenant')
				);
		...
		if ($acl->check('.invoice', ACL_READ, 'property'))
		{
			...
			$invoice = array_merge(array(
				...
				'deposition'	 => array(
					'url'	 => phpgw::link('/index.php', array(
						'menuaction' => 'property.uiinvoice.reporting',
						'type'		 => 'deposition'
					)),
		...
		if (is_array($entity_list) && count($entity_list))
		{
			foreach ($entity_list as $entry)
			{
				if ($acl->check(".entity.{$entry['id']}", ACL_READ, 'property'))
				{
					...
					$custom_menu_items = $custom_menus->read_tree(array(
						'type'	 => 'custom_menu_items',
