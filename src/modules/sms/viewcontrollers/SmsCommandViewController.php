...
class SmsCommandViewController
{
	...
	private function render(Request $request, Response $response, string $template, array $data, string $menuSelection): Response
	{
		...
		{
			$storage = null;
			$guard = new Guard(new \Slim\Psr7\Factory\ResponseFactory(), 'csrf', $storage, null, 200, 16, true);
			$request = $guard->appendNewTokenToRequest($request);
			$csrfName = (string) $request->getAttribute('csrf_name');
			$csrfValue = (string) $request->getAttribute('csrf_value');
		}
	...
	public function redirect(Request $request, Response $response): Response
	{
		...
		if (!is_array($link_data) || empty($link_data['menuaction']))
		{
			return ResponseHelper::sendErrorResponse(['error' => 'Command redirect is not configured'], 404);
		}
