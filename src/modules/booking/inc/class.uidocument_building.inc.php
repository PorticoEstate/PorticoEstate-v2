				...
				if (!$errors)
				{
					try
					{
						...
						if (class_exists('\App\modules\bookingfrontend\services\CacheService'))
						{
							...
							// owner_id is the building_id for building documents
							$buildingId = isset($document['owner_id']) ? (int)$document['owner_id'] : null;
							if ($buildingId)
							{
								$cache->invalidateBuildingDocuments($buildingId);
							}
						}
