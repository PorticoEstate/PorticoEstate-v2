...
foreach ($modules as $moduleDir)
{
	...
	foreach ($files as $file)
	{
		...
		$translationPatterns = [
			// Modified pattern to preserve colon presence/absence
			'/>\s*([A-Z][a-zA-Z0-9_ ]+)(:)?\s*<\//' => function ($matches)
			{
				// Keep original colon if it existed, otherwise don't add one
				$colon = isset($matches[2]) ? ':' : '';
				return '> {{ lang(\'' . $matches[1] . '\') }}' . $colon . ' </';
			},
...
echo "Thank you for using the Template Converter!\n";
