...
//die();
	...
	function edit($claim)
	{
		...
		if (!empty($claim['takeover']))
		{
			if ($old_user_id != $this->account)
			{
				$accounts_obj = new Accounts();
				$value_set['user_id'] = $this->account;
				$historylog->add('RM', $claim_id, $accounts_obj->get($this->account)->__toString(), $accounts_obj->get($old_user_id)->__toString());
			}
		}
