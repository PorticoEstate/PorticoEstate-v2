	...
	return (
		<div className={styles.demoContainer}>
			...
			<div className={styles.statusIndicator}>
				...
				className={connectionStatus === 'OPEN' ? styles.connected : styles.disconnected}>
          {connectionStatus}
        </span>
			</div>
				...
				<ul>
					{messages.map((msg, index) => (
						<li key={index} className={styles.message}>
							...
							{'message' in msg && (<div><strong>Message:</strong> {msg.message}</div>)}

							<div><strong>Timestamp:</strong> {msg.timestamp}</div>
							{msg.type === 'entity_event' && (
								<div className={styles.entityData}>
									<div><strong>Entity:</strong> {(msg as any).entityType} {(msg as any).entityId}
									</div>
