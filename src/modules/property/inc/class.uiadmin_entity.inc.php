...
class property_uiadmin_entity extends phpgwapi_uicommon_jquery
{
	...
	function list_attribute_group()
	{
		...
		$data['datatable']['actions'][] = array(
			...
			'text'		 => lang('cancel'),
			'custom_code'	 => 'window.open("' . self::link(
				array(
					'menuaction' => 'property.uiadmin_entity.category',
					'entity_id'	 => $entity_id,
					'type'		 => $this->type
	...
	function convert_to_eav()
	{
		...
		if ($values['confirm_convert_to_eav'] && $values['locations'])
		{
			$this->bo->convert_to_eav($values['locations']);
		}


		//first: list all categories with entities which is not eav, and ask user if he really wants to convert to eav.
		// if user confirms, convert all categories with entities to eav, and delete old tables with entities and categories.
		$non_eav = array();
		$location_obj = new Locations();
		$entities = $this->bo->read(array('allrows' => true));
		foreach ($entities as $entity)
		{
			$categories = $this->bo->read_category(array('allrows' => true, 'entity_id' => $entity['id']));
			foreach ($categories as $category)
			{
				if (!$category['is_eav'])
				{
					$non_eav[] = array(
						'entity_name' => $entity['name'],
						'category_name' => $category['name'],
						'location' => $location_obj->get_location($category['location_id'])
					);
				}
			}
		}
		...
		self::render_template_xsl(array('admin_entity'), array(
			'convert_to_eav' => $data
		));
	}
