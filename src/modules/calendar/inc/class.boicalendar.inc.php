		...
		for ($i = 0; $i < $c_events; $i++)
		{
			...
			if (count($ical['event'][$i]['alarm']) > 0)
			{
				for ($a = 0; $a < count($ical['event'][$i]['alarm']); ++$a)
				{
					...
					$imported_alarm[$a] = array(
						'time' => mktime($ical['event'][$i]['dtstart']['hour'] - $alarm_prior_hours, $ical['event'][$i]['dtstart']['min'] - $alarm_prior_min + $offset_mins, $ical['event'][$i]['dtstart']['sec'], $ical['event'][$i]['dtstart']['month'], $ical['event'][$i]['dtstart']['mday'] - $alarm_prior_days, $ical['event'][$i]['dtstart']['year']),
