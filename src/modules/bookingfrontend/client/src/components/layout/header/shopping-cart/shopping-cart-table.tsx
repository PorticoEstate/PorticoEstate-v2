					...
					return (
						<div key={item.id} className={styles.mobileTableCard}>
							...
							<div className={styles.mobileCardActions}>
								<Button variant="tertiary" data-size="sm" onClick={() => openEdit(item)}
										aria-label={i18n.t('bookingfrontend.edit')}>
									<PencilIcon/> {i18n.t('bookingfrontend.edit')}
								</Button>
								<Button variant="tertiary" data-size="sm"
										onClick={() => deletePartialApplication(item.id)}
										aria-label={i18n.t('bookingfrontend.delete')}>
		...
		>
			<Table.Head>
				<Table.Row>
					...
					{!showParentSelection && <Table.HeaderCell>{i18n.t('bookingfrontend.where')}</Table.HeaderCell>}
			...
			<Table.Body>
				{basketData.map((item) => (
					<Table.Row key={item.id} onClick={() => {
						...
						{!showParentSelection && <Table.Cell>{item.building_name}</Table.Cell>}
						<Table.Cell>
							<ResourceCircles
								resources={item.resources || []}
								maxCircles={4}
						...
						<Table.Cell>
							<div style={{display: 'flex', gap: '0.5rem'}}>
								<Button variant="tertiary" className={'link-text link-text-unset normal'}
										onClick={(e) => {
											openEdit(item);
											e.stopPropagation();
										}} aria-label={i18n.t('bookingfrontend.edit')}>
