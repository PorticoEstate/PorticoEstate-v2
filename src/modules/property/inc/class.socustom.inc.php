...
class property_socustom
{
	...
	function read_custom($data)
	{
		...
		$_sql = "SELECT * FROM ({$sql}) custom_query";

		if ($query && !$update)
		{
			foreach ($uicols as $uicol)
			{
				$column_name = trim($uicol['name']);
				...
				$filter_arr[] = "CAST(custom_query.{$column_name} AS text) {$this->db->like} '{$query}%'";
			}

			if ($filter_arr)
			{
				$_sql .= ' WHERE (' . implode(' OR ', $filter_arr) . ')';
			}
		}

		if ($order && preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/', $order))
		{
			$sort = strtoupper($sort) === 'ASC' ? 'ASC' : 'DESC';
			$ordermethod = " ORDER BY custom_query.{$order} {$sort}";
		}
