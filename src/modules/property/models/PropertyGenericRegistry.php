 ...
 * Self-contained with all registry definitions included
 */
class PropertyGenericRegistry extends GenericRegistry
{
	/**
	 * Properties needed for get_location_info compatibility
	 */
	public $type;
	public $type_id;
	...
	/**
	 * Constructor override to initialize database and account properties
	 */
	 ...
	 * Uses method-based approach with internal get_location_info method
	 */
	protected static function loadRegistryDefinitions(): void
	{
		// Initialize empty array - definitions will be loaded on-demand
		static::$registryDefinitions = [];
	}

	/**
	 * Get registry definition for a specific type
	 * Uses internal get_location_info method instead of legacy property_sogeneric
	 * 
	 * @param string $type The registry type
	 * @param int $type_id Optional type ID for certain types
	 * @return array|null The registry definition or null if not found
	 */
	public static function getRegistryDefinition(string $type, int $type_id = 0): ?array
	{
		...
		if (isset(static::$registryDefinitions[$key])) {
			return static::$registryDefinitions[$key];
		}
		...
		try {
			...
			if (!empty($info)) {
				...
				static::$registryDefinitions[$key] = static::convertLegacyDefinition($info);
				return static::$registryDefinitions[$key];
			}
		} catch (\Exception $e) {
			// Log error but don't break
			error_log("PropertyGenericRegistry: Failed to load definition for type '{$type}': " . $e->getMessage());
		}
	...
	/**
	 * Convert legacy property_sogeneric definition to our format
	 * 
	 * @param array $legacyInfo Legacy definition from property_sogeneric::get_location_info()
	 * @return array Converted definition
	 */
	private static function convertLegacyDefinition(array $legacyInfo): array
	{
		$converted = [
			'table' => $legacyInfo['table'] ?? '',
			'name' => $legacyInfo['name'] ?? '',
			'id' => $legacyInfo['id'] ?? ['name' => 'id', 'type' => 'int'],
			'acl_app' => $legacyInfo['acl_app'] ?? 'property',
			'acl_location' => $legacyInfo['acl_location'] ?? '.admin',
			'menu_selection' => $legacyInfo['menu_selection'] ?? '',
			'fields' => []
		];

		// Convert optional properties
		if (isset($legacyInfo['edit_msg'])) {
			$converted['edit_msg'] = $legacyInfo['edit_msg'];
		}
		if (isset($legacyInfo['add_msg'])) {
			$converted['add_msg'] = $legacyInfo['add_msg'];
		}
		if (isset($legacyInfo['default'])) {
			$converted['default'] = $legacyInfo['default'];
		}
		if (isset($legacyInfo['custom_criteria'])) {
			$converted['custom_criteria'] = $legacyInfo['custom_criteria'];
		}
		if (isset($legacyInfo['system_location'])) {
			$converted['system_location'] = $legacyInfo['system_location'];
		}
		if (isset($legacyInfo['check_grant'])) {
			$converted['check_grant'] = $legacyInfo['check_grant'];
		}

		// Convert fields
		if (isset($legacyInfo['fields']) && is_array($legacyInfo['fields'])) {
			foreach ($legacyInfo['fields'] as $field) {
				$convertedField = [
					'name' => $field['name'] ?? '',
				...
				if (isset($field['descr'])) {
					$convertedField['descr'] = $field['descr'];
				}
				if (isset($field['nullable'])) {
					$convertedField['nullable'] = $field['nullable'];
				}
				if (isset($field['size'])) {
					$convertedField['maxlength'] = $field['size'];
				}
				if (isset($field['default'])) {
					$convertedField['default'] = $field['default'];
				}
				if (isset($field['filter'])) {
					$convertedField['filter'] = $field['filter'];
				}
				if (isset($field['sortable'])) {
					$convertedField['sortable'] = $field['sortable'];
				}
				if (isset($field['hidden'])) {
					$convertedField['hidden'] = $field['hidden'];
				}
				if (isset($field['values_def'])) {
					$convertedField['values_def'] = $field['values_def'];
				}
				if (isset($field['js_file'])) {
					$convertedField['js_file'] = $field['js_file'];
				}

				$converted['fields'][] = $convertedField;
			}
		}
	...
	/**
	 * Override parent method to use our custom getRegistryDefinition
	 */
	public static function getDefinition(string $type, int $type_id = 0): ?array
	{
		if (empty(static::$registryDefinitions)) {
			static::loadRegistryDefinitions();
		}

		return static::getRegistryDefinition($type, $type_id);
	}

	/**
	 * Override parent getRegistryConfig to integrate with GenericRegistry base class
	 * This ensures PropertyGenericRegistry works seamlessly with all GenericRegistry methods
	 */
	...
	/**
	 * Enhanced forType method that handles type_id for certain registry types
	 */
	public static function forType(string $registryType, array $data = []): static
	{
		// Extract type_id if embedded in registryType (e.g., "location_1")
		$type_id = 0;
		if (str_contains($registryType, '_') && is_numeric(substr($registryType, strrpos($registryType, '_') + 1))) {
			$parts = explode('_', $registryType);
			$type_id = (int)array_pop($parts);
			$registryType = implode('_', $parts);
		}

		$instance = new static($registryType, $data);
		
		// For types that require type_id, fetch the specific definition
		if ($type_id > 0) {
			$instance->registryConfig = static::getRegistryDefinition($registryType, $type_id) ?? [];
		}
	...
	/**
	 * Get all available registry types from legacy property_sogeneric
	 * This is useful for discovering all available types
	 * 
	 * @return array List of available type names
	 */
	public static function getAvailableTypes(): array
	{
		// Known types from property_sogeneric switch statement
		return [
			'part_of_town',
			'dimb',
			'dimd', 
			...
			'ticket_priority',
			'external_com_type',
			'order_template_delivery_type',
			'order_template_payment_type',
			'zip_code',
			'external_project',
			'unspsc_code',
			'project_status',
	 ...
	 * 
	 * @param string $type The registry type to check
	 * @param int $type_id Optional type ID for certain types
	 * @return bool True if type exists, false otherwise
	 */
	public static function hasType(string $type, int $type_id = 0): bool
	{
		return static::getRegistryDefinition($type, $type_id) !== null;
	}
	...
	/**
	 * Get location info - legacy compatibility method
	 * 
	 * @param string $type The registry type
	 * @param int $type_id Optional type ID
	 * @return array|null Registry definition
	 */
	public static function getLocationInfo(string $type, int $type_id = 0): ?array
	{
		return static::getRegistryDefinition($type, $type_id);
	}
	...
	public function get_location_info($type, $type_id = 0)
	{
		...
		switch ($type)
		{
			...
			case 'location':
				...
				$stmt = $this->_db->prepare("SELECT id FROM fm_location_type WHERE id = :type_id");
